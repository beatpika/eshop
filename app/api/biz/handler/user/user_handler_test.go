...
func TestUpdatePassword(t *testing.T) {
	h := server.Default()
	h.PUT("/user/password", UpdatePassword)
...
func TestUpdatePhone(t *testing.T) {
	h := server.Default()
	h.PUT("/user/phone", UpdatePhone)
	path := "/user/phone"                                     // todo: you can customize query
	body := &ut.Body{Body: bytes.NewBufferString(""), Len: 1} // todo: you can customize body
	header := ut.Header{}                                     // todo: you can customize header
	w := ut.PerformRequest(h.Engine, "PUT", path, body, header)
	resp := w.Result()
...
func TestDeactivateAccount(t *testing.T) {
	h := server.Default()
	h.POST("/user/deactivate", DeactivateAccount)
	path := "/user/deactivate"                                // todo: you can customize query
	body := &ut.Body{Body: bytes.NewBufferString(""), Len: 1} // todo: you can customize body
	header := ut.Header{}                                     // todo: you can customize header
	w := ut.PerformRequest(h.Engine, "POST", path, body, header)
	resp := w.Result()
